...
import { AppointmentRow } from "../admin/AdminAppointments";
  ...
  const fetch = () =>
    api.get("/appointments").then((r) => {
      ...
      const mine = r.data.filter(
        (a) => a.doctorName.toLowerCase() === user.name.toLowerCase()
      );
    ...
    <div className="page">
      ...
      <div className="records-list">
        {filtered.length === 0
          ...
          : filtered.map((a) => (
            <AppointmentRow key={a._id} appt={a} onStatus={handleStatus} onDelete={() => {}} isDoctor />
          ))
        }
      </div>
    </div>
